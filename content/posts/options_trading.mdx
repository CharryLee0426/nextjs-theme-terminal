...
\begin{itemize}
\item Close losing positions at 50% of max loss
\item Take profits at 50-75% of max profit (don't wait for expiration)
\item Use mental or hard stops to enforce discipline
\end{itemize}
...
\begin{itemize}
\item Be aware of time decay as expiration approaches
\item Theta (time decay) accelerates in final week before expiration
\item Don't wait until last day unless you have a specific plan
\item Close positions 1-2 weeks before expiration to avoid assignment surprises
\end{itemize}
...
\begin{enumerate}
\item \textbf{Buying out-of-the-money options} - Low probability, money disappears quickly
\item \textbf{Holding through expiration} - Don't wait; close early to avoid assignment surprises
\item \textbf{Ignoring Greeks} - Delta, Theta, and Vega matter for price predictions
\item \textbf{Over-leveraging} - Risk only 2-5% per trade
\item \textbf{Not understanding assignment} - Short options can be assigned unexpectedly
\item \textbf{Selling naked calls} - Unlimited loss potential; requires portfolio margin
\item \textbf{Picking earnings plays blindly} - Volatility crush is real after earnings
\item \textbf{Not tracking probability} - Use delta as proxy for probability of profit
\end{enumerate}
